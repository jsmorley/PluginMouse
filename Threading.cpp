...
// A single pending message entry
struct PendingMsg {
	wchar_t* buffer;  
...
static std::vector<PendingMsg>  g_pendingMsgs;

void SendDeferredNotifyMessage(HWND hwnd, UINT msg, WPARAM wParam, const std::wstring& action)
{
	size_t len = action.size() + 1;  
	wchar_t* buf = new wchar_t[len];
	memcpy(buf, action.c_str(), len * sizeof(wchar_t));

	PendingMsg pm;
	pm.buffer = buf;
	pm.expiry = std::chrono::steady_clock::now()
	...
	{  
		...
		g_pendingMsgs.push_back(pm);
	}

	SendNotifyMessageW(hwnd, msg, wParam, (LPARAM)buf);
}
	...
	while (1)
	{
		...
		if (bRet > 0)
		{
			...
			for (Measure* measure : Measures)
			{
				...
				if (!actions.empty())
				{
					for (wstring action : actions)
					{
						...
						SendDeferredNotifyMessage(hMainWindow,
							WM_RAINMETER_EXECUTE,
							(WPARAM)measure->skin,
							action);
					}
					if (measure->delay > 16)
			...
			DispatchMessage(&msg);
			{
				...
				std::lock_guard<std::mutex> lk(g_pendingMutex);
				for (auto it = g_pendingMsgs.begin(); it != g_pendingMsgs.end(); )
				{
					if (it->expiry <= now)
					{
						delete[] it->buffer;
						it = g_pendingMsgs.erase(it);
					}
					else
						++it;
				}
